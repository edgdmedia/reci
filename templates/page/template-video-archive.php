 ...
 *
 * Video archive using query/format services.
 *
...
$page_title    = 'Videos';
$page_subtitle = 'Watch documentary films, panel discussions, and educational sessions exploring justice, equity, and systemic change.';
...
$base_url        = is_post_type_archive('reci_video') ? get_post_type_archive_link('reci_video') : get_permalink();
$base_url        = $base_url ?: home_url('/videos/');
...
$video_listing = RECI_Post_Query_Service::get_formatted_items($query_args, [
	'image_size'    => 'large',
	'tag_limit'     => 3,
	'excerpt_words' => 16,
]);
$video_items = $video_listing['items'];
$video_query = $video_listing['query'];
...
<main class="layout-page">
	...
	<section class="reci-container pt-5 pb-14 flex flex-col justify-start items-start gap-10">
		<div class="self-stretch pb-5 border-b border-zinc-400">
			<form method="get" action="<?php echo esc_url($base_url); ?>" class="self-stretch flex flex-col sm:flex-row justify-between items-center gap-5" data-archive-filter-form data-search-min="3" data-search-debounce="350">
				<div class="flex justify-start items-center gap-5 flex-wrap">
					...
					<div class="relative">
						<label for="video-topic-filter" class="sr-only">Filter by topic</label>
						<select id="video-topic-filter" name="topic" class="appearance-none px-4 py-2 pr-8 text-neutral-800 text-base font-normal bg-transparent border-none cursor-pointer focus:outline-none" aria-label="Filter by topic">
					...
					<?php if (! empty($sphere_terms_filter)) : ?>
						<div class="relative">
							<label for="video-sphere-filter" class="sr-only">Filter by sphere</label>
							<select id="video-sphere-filter" name="sphere" class="appearance-none px-4 py-2 pr-8 text-neutral-800 text-base font-normal bg-transparent border-none cursor-pointer focus:outline-none" aria-label="Filter by sphere">
				...
				<div class="w-full sm:w-auto flex items-center gap-2.5">
					<div class="w-full sm:w-80 px-5 py-4 bg-white rounded-lg border border-zinc-300 flex justify-start items-center gap-2.5" role="search">
						...
						<input id="video-search" type="search" name="search" value="<?php echo esc_attr($current_search); ?>" placeholder="Search Videos" class="flex-1 italic bg-transparent text-neutral-600 text-base font-light placeholder-neutral-600 focus:outline-none" />
					</div>
					<?php if ($has_filters) : ?>
		...
		<?php if (! empty($video_items)) : ?>
			...
			<?php echo RECI_Post_Query_Service::render_pagination($video_query, [
				'base_url'      => $base_url,
				'param_name'    => 'paged',
		...
		<?php else : ?>
			<p class="text-neutral-600 text-base">No videos found for this filter combination.</p>
		<?php endif; ?>
	</section>
</main>
