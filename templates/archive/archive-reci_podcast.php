<?php

/**
 * Native archive route for reci_podcast.
 *
 * @package reci-media-hub
 */

if (! defined('ABSPATH')) {
	exit;
}

$page_title    = 'Podcasts';
$page_subtitle = 'Conversations that illuminate the issues shaping our communities — from racial equity and digital access to environmental justice and cultural identity.';

$topic_terms = get_terms([
	'taxonomy'   => 'category',
	'hide_empty' => true,
]);
if (is_wp_error($topic_terms)) {
	$topic_terms = [];
}

$sphere_terms_filter = get_terms([
    'taxonomy'   => 'reci_sphere',
    'hide_empty' => true,
]);
if (is_wp_error($sphere_terms_filter)) {
    $sphere_terms_filter = [];
}

$show_terms_filter = get_terms([
    'taxonomy'   => 'reci_show',
    'hide_empty' => true,
]);
if (is_wp_error($show_terms_filter)) {
    $show_terms_filter = [];
}

$current_topic  = isset($_GET['topic']) ? sanitize_title((string) wp_unslash($_GET['topic'])) : '';
$current_sphere = isset($_GET['sphere']) ? sanitize_title((string) wp_unslash($_GET['sphere'])) : '';
$current_show   = isset($_GET['show']) ? sanitize_title((string) wp_unslash($_GET['show'])) : '';
$current_search = isset($_GET['search']) ? sanitize_text_field((string) wp_unslash($_GET['search'])) : '';

$base_url        = is_post_type_archive('reci_podcast') ? get_post_type_archive_link('reci_podcast') : get_permalink();
$base_url        = $base_url ?: home_url('/podcasts/');
$all_filters_url = remove_query_arg(['topic', 'sphere', 'show', 'search', 'paged'], $base_url);
$has_filters     = ($current_topic !== '') || ($current_sphere !== '') || ($current_show !== '') || ($current_search !== '');

$podcasts_per_page = (int) reci_setting('content_podcasts_per_page', 9);

$query_args = [
	'post_type'      => 'reci_podcast',
	'post_status'    => 'publish',
	'posts_per_page' => $podcasts_per_page,
	'paged'          => max(1, (int) get_query_var('paged'), (int) get_query_var('page')),
	'orderby'        => 'date',
	'order'          => 'DESC',
];
if ($current_search !== '') {
	$query_args['s'] = $current_search;
}
$query_args['tax_query'] = ['relation' => 'AND'];
if ($current_topic !== '') {
	$query_args['tax_query'][] = [
		'taxonomy' => 'category',
		'field'    => 'slug',
		'terms'    => [$current_topic],
	];
}
if ($current_sphere !== '') {
	$query_args['tax_query'][] = [
		'taxonomy' => 'reci_sphere',
		'field'    => 'slug',
		'terms'    => [$current_sphere],
	];
}
if ($current_show !== '') {
	$query_args['tax_query'][] = [
		'taxonomy' => 'reci_show',
		'field'    => 'slug',
		'terms'    => [$current_show],
	];
}

$podcast_listing = RECI_Post_Query_Service::get_formatted_items($query_args, [
	'image_size'    => 'large',
	'tag_limit'     => 3,
	'excerpt_words' => 16,
]);
$podcast_items = $podcast_listing['items'];
$podcast_query = $podcast_listing['query'];

get_header();
?>

<main class="layout-page">
	<?php get_template_part('template-parts/common/page-title-card', null, [
		'title'    => $page_title,
		'subtitle' => $page_subtitle,
	]); ?>

	<section class="reci-container pt-5 pb-14 flex flex-col justify-start items-start gap-10">
		<div class="self-stretch pb-5 border-b border-zinc-400">
			<form method="get" action="<?php echo esc_url($base_url); ?>" class="self-stretch flex flex-col sm:flex-row justify-between items-center gap-5" data-archive-filter-form data-search-min="3" data-search-debounce="350">
				<div class="flex justify-start items-center gap-5 flex-wrap">
					<span class="text-neutral-800 text-base font-bold">Filter by:</span>
					<div class="archive-filter-select-wrap">
						<label for="podcast-topic-filter" class="sr-only">Filter by topic</label>
						<select id="podcast-topic-filter" name="topic" class="archive-filter-select" aria-label="Filter by topic">
							<option value="">All</option>
							<?php foreach ($topic_terms as $term) : ?>
								<option value="<?php echo esc_attr($term->slug); ?>" <?php selected($current_topic, $term->slug); ?>><?php echo esc_html($term->name); ?></option>
							<?php endforeach; ?>
						</select>
						<span class="archive-filter-chevron">
							<svg class="w-4 h-4 text-neutral-800" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" aria-hidden="true">
								<path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" />
							</svg>
						</span>
					</div>
					<?php if (! empty($sphere_terms_filter)) : ?>
						<div class="archive-filter-select-wrap">
							<label for="podcast-sphere-filter" class="sr-only">Filter by sphere</label>
							<select id="podcast-sphere-filter" name="sphere" class="archive-filter-select" aria-label="Filter by sphere">
								<option value="">All Spheres</option>
                                <?php foreach ($sphere_terms_filter as $st):
                $st_name = $st->name;
                ?>
									<option value="<?php echo esc_attr($st->slug); ?>" <?php selected($current_sphere, $st->slug); ?>><?php echo esc_html($st_name); ?></option>
								<?php endforeach; ?>
							</select>
							<span class="archive-filter-chevron">
								<svg class="w-4 h-4 text-neutral-800" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" aria-hidden="true">
									<path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" />
								</svg>
							</span>
						</div>
					<?php endif; ?>
					<?php if (! empty($show_terms_filter)) : ?>
						<div class="archive-filter-select-wrap">
							<label for="podcast-show-filter" class="sr-only">Filter by show</label>
							<select id="podcast-show-filter" name="show" class="archive-filter-select" aria-label="Filter by show">
								<option value="">All Shows</option>
								<?php foreach ($show_terms_filter as $st) : ?>
									<option value="<?php echo esc_attr($st->slug); ?>" <?php selected($current_show, $st->slug); ?>><?php echo esc_html($st->name); ?></option>
								<?php endforeach; ?>
							</select>
							<span class="archive-filter-chevron">
								<svg class="w-4 h-4 text-neutral-800" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" aria-hidden="true">
									<path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" />
								</svg>
							</span>
						</div>
					<?php endif; ?>
				</div>

				<div class="w-full sm:w-auto flex items-center gap-2.5">
					<div class="archive-filter-search-wrap" role="search">
						<svg class="w-4 h-4 flex-shrink-0 text-neutral-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
						</svg>
						<label for="podcast-search" class="sr-only">Search podcasts</label>
						<input id="podcast-search" type="search" name="search" value="<?php echo esc_attr($current_search); ?>" placeholder="Search Podcasts" class="archive-filter-search-input" />
					</div>
					<?php if ($has_filters) : ?>
						<a href="<?php echo esc_url($all_filters_url); ?>" class="px-4 py-3 text-sm font-medium text-neutral-700 hover:text-neutral-900">Reset</a>
					<?php endif; ?>
				</div>
			</form>
		</div>

		<?php if (! empty($podcast_items)) : ?>
			<div class="self-stretch grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
				<?php foreach ($podcast_items as $item) : ?>
					<?php get_template_part('template-parts/listings/podcast-archive-card', null, $item); ?>
				<?php endforeach; ?>
			</div>
			<?php echo RECI_Post_Query_Service::render_pagination($podcast_query, [
				'base_url'      => $base_url,
				'param_name'    => 'paged',
				'wrapper_class' => 'mt-8 flex w-full items-center justify-center gap-2',
				'item_class'    => "inline-flex items-center justify-center min-w-11 h-11 px-3 rounded-lg border border-zinc-300 text-sm font-medium text-neutral-800 hover:bg-zinc-100",
				'current_class' => "inline-flex items-center justify-center min-w-11 h-11 px-3 rounded-lg bg-[#003594] text-sm font-medium text-white",
			]); ?>
		<?php else : ?>
			<p class="text-neutral-600 text-base">No podcasts found for this filter combination.</p>
		<?php endif; ?>
	</section>
</main>

<?php get_footer(); ?>
