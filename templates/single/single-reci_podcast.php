...
$podcast_spheres = reci_get_post_spheres($post_id);

$show_terms = wp_get_post_terms($post_id, 'reci_show');
if (is_wp_error($show_terms)) {
	$show_terms = [];
}
...
$host = [
	'name'      => (string) ($display_author['name'] ?? ''),
	'title'     => (string) ($display_author['title'] ?? __('RECI Media Hub', 'reci-media-hub')),
	'bio'       => (string) ($display_author['bio'] ?? ''),
	'avatar'    => (string) ($display_author['image_url'] ?? ''),
	'permalink' => (string) ($display_author['permalink'] ?? ''),
];
...
<div class="layout-page">
	<div class="reci-container">
		...
		<div class="flex flex-col lg:flex-row justify-start items-start gap-0 ">
			...
			<div class="w-full  py-14 px-10 flex flex-col justify-start items-start gap-10 border-x border-zinc-300">
				...
				<div class="self-stretch flex flex-col justify-start items-start gap-5">
					...
					<div class="flex items-center justify-between w-full flex-wrap gap-4">
						<div class="inline-flex justify-start items-center gap-2.5 flex-wrap">
							<a href="<?php echo esc_url(get_post_type_archive_link('reci_podcast')); ?>" class="no-underline">
								<div class="px-2 py-1 bg-neutral-800 rounded flex justify-center items-center gap-2.5">
									<span class="text-white text-sm font-normal leading-4"><?php esc_html_e('Podcast', 'reci-media-hub'); ?></span>
								</div>
							</a>
							<?php if ($category !== '') : ?>
							...
							<?php endif; ?>
							<?php if (! empty($show_terms)) : ?>
								<div class="w-2 h-2 bg-amber-400 rounded-sm"></div>
								<a href="<?php echo esc_url(get_term_link($show_terms[0])); ?>" class="text-neutral-500 text-sm font-normal leading-4 no-underline hover:underline"><?php echo esc_html($show_terms[0]->name); ?></a>
							<?php endif; ?>
							<?php if ($ep_num || $season_num) : ?>
								<div class="w-2 h-2 bg-amber-400 rounded-sm"></div>
								<span class="text-neutral-500 text-sm font-normal leading-4">
									<?php
							...
							<?php if (! empty($host['name'])) : ?>
								...
								<p class="font-bold font-subhead text-neutral-600">By
									<a href="<?php echo esc_url($host['permalink'] ?: get_author_posts_url((int) get_post_field('post_author', $post_id))); ?>" class="text-neutral-600 hover:underline"><?php echo esc_html($host['name']); ?></a>
								</p>
							<?php endif; ?>
							<div class="tag-dot"></div>
							...
							<?php if ($duration) : ?>
								...
								<div class="flex justify-start items-center gap-1">
									<?php echo reci_inline_svg('assets/icons/headphones.svg', 'w-3.5 h-3.5 flex-shrink-0 opacity-60 text-neutral-600', ['aria-hidden' => 'true']); ?>
									<span class="text-neutral-600 text-sm font-normal"><?php echo esc_html($duration); ?></span>
								</div>
							<?php endif; ?>
						</div>
					...
					<div class="self-stretch flex flex-col justify-start items-start gap-5">
						...
						<?php if ($excerpt) : ?>
							<p class="self-stretch text-neutral-500 text-lg font-normal leading-7 ">
...
<?php endif; ?>
				...
				<?php if ($audio_url) : ?>
					...
					<div class="self-stretch flex flex-col gap-2" data-audio-player>
						<div class="self-stretch inline-flex justify-start items-center gap-2.5">
							...
							<span class="text-neutral-500 text-xs font-normal tabular-nums flex-shrink-0" data-audio-time>
				...
				<?php if ($spotify || $apple || $transcript) : ?>
					<div class="inline-flex flex-wrap items-center gap-x-4 gap-y-1">
						<?php if ($spotify) : ?>
							<a href="<?php echo esc_url($spotify); ?>" target="_blank" rel="noopener noreferrer" class="text-neutral-500 text-xs font-normal hover:text-neutral-800 transition-colors inline-flex items-center gap-1">
						...
						<?php endif; ?>
						<?php if ($apple) : ?>
							<a href="<?php echo esc_url($apple); ?>" target="_blank" rel="noopener noreferrer" class="text-neutral-500 text-xs font-normal hover:text-neutral-800 transition-colors inline-flex items-center gap-1">
								<svg class="w-3.5 h-3.5 flex-shrink-0" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
						...
						<?php endif; ?>
						<?php if ($transcript) : ?>
							<a href="<?php echo esc_url($transcript); ?>" target="_blank" rel="noopener noreferrer" class="text-neutral-500 text-xs font-normal hover:text-neutral-800 transition-colors inline-flex items-center gap-1">
...
<?php endif; ?>
			...
			<div class="w-full md:w-1/3 pl-5 pt-14 pb-10 flex flex-col justify-start items-start gap-2.5 lg:sticky top-10">
				<div class="flex-1 p-5 bg-neutral-800 flex flex-col justify-start items-start gap-10 overflow-hidden rounded-lg">
					...
					<div class="self-stretch flex flex-col justify-start items-start gap-5">
						<div class="self-stretch inline-flex justify-start items-center gap-2">
							<div class="w-2 h-2 bg-amber-400 rounded-sm"></div>
							<h2 class="text-white text-2xl font-heading leading-7"><?php esc_html_e('Related Podcast', 'reci-media-hub'); ?></h2>
						</div>
					...
					<?php if (! empty($related_episodes)) : ?>
						<div class="self-stretch flex flex-col justify-start items-start gap-10 overflow-hidden">
							<?php foreach ($related_episodes as $idx => $rel) : ?>
								...
								<?php endif; ?>
								<?php get_template_part('template-parts/listings/podcast-sidebar-card', null, $rel); ?>
							<?php endforeach; ?>
						</div>
					<?php else : ?>
						<p class="text-zinc-400 text-sm font-normal"><?php esc_html_e('No related episodes yet.', 'reci-media-hub'); ?></p>
					<?php endif; ?>
					...
					<div class="flex-1 flex flex-col justify-start items-start gap-5">
						...
						<div class="self-stretch flex flex-col justify-start items-start gap-5">
							<div class="inline-flex justify-start items-center gap-2">
								<div class="w-2 h-2 bg-amber-400 rounded-sm"></div>
								<h2 class="text-neutral-800 text-xl font-bold font-subhead leading-6"><?php esc_html_e('Share this podcast', 'reci-media-hub'); ?></h2>
							</div>
				...
				<?php if ($host['name']) : ?>
					...
					<div class="w-full px-4 pt-10 flex flex-col justify-start items-start gap-6">
						...
						<div class="self-stretch flex flex-col justify-start items-start gap-5">
							<div class="inline-flex justify-start items-center gap-2">
								<div class="w-2 h-2 bg-amber-400 rounded-sm"></div>
								<h2 class="text-neutral-800 text-xl font-bold font-subhead leading-6"><?php esc_html_e('Hosted by', 'reci-media-hub'); ?></h2>
							</div>
							<div class="self-stretch h-px bg-zinc-300"></div>
						</div>
						...
						<div class="flex flex-col md:flex-row justify-start items-start gap-4">
							...
							<div>
								<?php if ($host['permalink']) : ?>
									<a href="<?php echo esc_url($host['permalink']); ?>" class="text-neutral-800 text-2xl font-bold font-subhead leading-7 hover:underline">
										<?php echo esc_html($host['name']); ?>
									</a>
								<?php else : ?>
									<h2 class="text-neutral-800 text-2xl font-bold font-subhead leading-7"><?php echo esc_html($host['name']); ?></h2>
								<?php endif; ?>
								<?php if ($host['title']) : ?>
									<p class="text-neutral-500 text-sm font-normal leading-5"><?php echo esc_html($host['title']); ?></p>
								<?php endif; ?>
								<?php if ($host['bio']) : ?>
									<p class="text-neutral-500 text-base font-normal leading-6  max-w-prose"><?php echo esc_html($host['bio']); ?></p>
								<?php endif; ?>
							</div>
						</div>
		...
		<?php if (! empty($read_also_items)) : ?>
			<div class=" py-10 flex flex-col justify-start items-start gap-10 border-t border-zinc-300">
				<div class="self-stretch inline-flex justify-start items-center gap-5 border-b pb-5 border-zinc-300">
