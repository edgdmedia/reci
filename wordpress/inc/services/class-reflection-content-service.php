...
if (! class_exists('RECI_Reflection_Content_Service')) {
    class RECI_Reflection_Content_Service {
        ...
        public static function get_payload(int $post_id): array {
            ...
            $speaker_user_id      = (int) get_post_meta($post_id, '_reci_reflection_speaker_user_id', true);
            ...
            $video_url            = (string) get_post_meta($post_id, '_reci_reflection_video_url', true);
            $audio_url            = (string) get_post_meta($post_id, '_reci_reflection_audio_url', true);
            ...
            if ($speaker_user_id > 0) {
                $user = get_user_by('id', $speaker_user_id);
                if ($user) {
                    $speaker = (string) $user->display_name;
                }
            }

            $role_terms = wp_get_post_terms($post_id, 'reci_reflection_role', ['fields' => 'names']);
            if (! is_wp_error($role_terms) && ! empty($role_terms)) {
                $role = (string) $role_terms[0];
            }

            $speaker = $speaker !== '' ? $speaker : get_the_title($post_id);
            $quote   = $quote !== '' ? $quote : (has_excerpt($post_id) ? get_the_excerpt($post_id) : wp_trim_words(wp_strip_all_tags((string) $post->post_content), 32, '...'));
