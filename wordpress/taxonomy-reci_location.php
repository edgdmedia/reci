...
$location_title = $term ? $term->name : 'Location';
...
<main class="bg-slate-100 min-h-screen">
	<section class="w-full bg-white border-b border-zinc-400">
		<div class="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-12 xl:px-20 py-14 flex flex-col lg:flex-row justify-between gap-6">
			<div class="flex items-center gap-3">
				...
				<h1 class="text-neutral-800 text-5xl font-medium font-['EB_Garamond']"><?php echo esc_html($location_title); ?></h1>
			</div>
			<?php if ($description !== '') : ?>
	...
	<section class="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-12 xl:px-20 py-6 border-b border-zinc-300">
		<form method="get" action="<?php echo esc_url($base_url); ?>" class="flex items-center justify-between gap-4 flex-wrap" data-archive-filter-form data-search-min="3" data-search-debounce="350">
			<div class="text-neutral-700 text-sm font-medium font-['SF_Pro_Display']">Filtering this location</div>
			<div class="flex items-center gap-2 w-full sm:w-auto">
				<label for="location-search-runtime" class="sr-only">Search in this location</label>
				<input id="location-search-runtime" type="search" name="search" value="<?php echo esc_attr($current_search); ?>" placeholder="Search this location" class="w-full sm:w-80 px-4 py-3 rounded-lg border border-zinc-300 bg-white text-neutral-700 text-base font-['SF_Pro_Display'] focus:outline-none" />
