			...
			<?php foreach ((array) $args['items'] as $index => $item) : ?>
				<div class="reci-timeline-panel" data-timeline-index="<?php echo esc_attr((string) $index); ?>">
					<article class="reci-timeline-card max-h-[76vh] overflow-y-auto">
						...
						<h2 class="mt-3 font-['Playfair_Display'] text-4xl font-semibold leading-tight text-[var(--reflection-text)] sm:text-5xl"><?php echo esc_html($item['title'] ?? ''); ?></h2>
						<p class="mt-5 max-w-[56rem] text-lg leading-8 text-[var(--reflection-soft-text)]"><?php echo esc_html($item['body'] ?? ''); ?></p>
						...
						<?php if (! empty($item['link'])) : ?>
							<p class="mt-5"><a class="font-semibold text-[var(--reflection-accent)] no-underline" href="<?php echo esc_url($item['link']['href']); ?>" target="_blank" rel="noopener"><?php echo esc_html($item['link']['label']); ?></a></p>
						<?php endif; ?>
					</article>
				</div>
			<?php endforeach; ?>
