...
$content_type_links = [
    "Articles" =>
        get_post_type_archive_link("reci_article") ?: home_url("/articles/"),
    "Videos" =>
...
<div
	...
	<div class="w-full border-b border-zinc-200">
		<div class="px-4 sm:px-6 h-24 flex justify-between items-center">
         ...
         $assets_url . rawurlencode("RECI Logo - Version 2 1.png"),
     ); ?>"
	...
	<div class="px-4 sm:px-6 grid grid-cols-1 md:grid-cols-2 md:gap-x-10">
		...
		<div>
			...
			<nav aria-label="RECI" class="pb-5 pt-3">
				...
				<a href="<?php echo esc_url(home_url('/learn/')); ?>" class="block pr-5 py-1 rounded-xl group">
			...
			<nav aria-label="Resources" class="pb-5 pt-3">
				<?php foreach ($content_type_links as $label => $url): ?>
					<?php if (in_array($label, ['Articles', 'Podcasts', 'Videos', 'Events'], true)): ?>
				...
				<a href="<?php echo esc_url(home_url('/learn/')); ?>" class="block pr-5 py-1 rounded-xl group">
					<span class="py-2 text-neutral-800 text-[24px] font-medium group-hover:text-amber-400 transition-colors">Training Modules</span>
				</a>
