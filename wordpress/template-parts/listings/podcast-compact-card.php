...
/**
 * Podcast compact card (from herov3.php lines 707-896 repeated variant).
 *
 * @var array $args
 */
...
<div data-layer="Content" class="Content flex-1 flex flex-col sm:flex-row justify-start items-start gap-5 px-2 sm:px-4 xl:px-6 py-6">
    <?php if ($image_url) : ?>
        <?php if ($link_url) : ?>
            <a href="<?php echo esc_url($link_url); ?>" class="w-full sm:w-44 self-stretch no-underline">
                <img data-layer="Image" class="Image w-full sm:w-44 self-stretch rounded-lg object-cover" src="<?php echo esc_url($image_url); ?>" alt="<?php echo esc_attr($image_alt); ?>" />
            </a>
        <?php else : ?>
            <img data-layer="Image" class="Image w-full sm:w-44 self-stretch rounded-lg object-cover" src="<?php echo esc_url($image_url); ?>" alt="<?php echo esc_attr($image_alt); ?>" />
        <?php endif; ?>
    ...
    <div data-layer="Content" class="Content flex-1 inline-flex flex-col justify-start items-start gap-6 lg:gap-8">
        ...
        <div data-layer="Content" class="Content self-stretch flex flex-col justify-start items-start gap-5">
            <div data-layer="content" class="Content self-stretch flex flex-col justify-start items-start gap-2">
                ...
                <?php if ($link_url) : ?>
                    <a href="<?php echo esc_url($link_url); ?>" data-layer="Title" class="TheDigitalDivideAccessAndEquity self-stretch justify-start text-neutral-800 text-xl font-bold font-['EB_Garamond'] leading-6 line-clamp-2 no-underline"><?php echo esc_html($title); ?></a>
                <?php else : ?>
                    <div data-layer="Title" class="TheDigitalDivideAccessAndEquity self-stretch justify-start text-neutral-800 text-xl font-bold font-['EB_Garamond'] leading-6 line-clamp-2"><?php echo esc_html($title); ?></div>
                <?php endif; ?>
            </div>
