...
$page_subtitle = 'Evaluate your understanding of key issues through guided assessments on equity, race, and social change.';
...
$base_url        = is_post_type_archive('reci_assessment') ? get_post_type_archive_link('reci_assessment') : get_permalink();
$base_url        = $base_url ?: home_url('/assessments/');
...
$listing_config = [
	...
	'empty_message'            => 'No assessments found for this filter combination.',
];
...
<div class="bg-slate-100 min-h-screen">
	<section class="w-full bg-white border-b border-zinc-400" aria-label="Assessments header">
		...
		<div class="self-stretch pb-5 border-b border-zinc-400">
			<form method="get" action="<?php echo esc_url($base_url); ?>" class="self-stretch flex flex-col sm:flex-row justify-between items-center gap-5" data-archive-filter-form data-search-min="3" data-search-debounce="350">
				<div class="flex justify-start items-center gap-5 flex-wrap">
					...
					<div class="relative">
						<label for="assessment-topic-filter" class="sr-only">Filter by topic</label>
						<select id="assessment-topic-filter" name="topic" class="appearance-none px-4 py-2 pr-8 text-neutral-800 text-base font-normal font-['SF_Pro_Display'] bg-transparent border-none cursor-pointer focus:outline-none" aria-label="Filter by topic">
				...
				<div class="w-full sm:w-auto flex items-center gap-2.5">
					<div class="w-full sm:w-80 px-5 py-4 bg-white rounded-lg flex justify-start items-center gap-2.5" role="search">
						...
						<input id="assessment-search" type="search" name="search" value="<?php echo esc_attr($current_search); ?>" placeholder="Search Assessments" class="flex-1 bg-transparent text-neutral-500 text-base font-light font-['SF_Pro_Display'] placeholder-neutral-500 focus:outline-none" />
					</div>
					<?php if ($has_filters) : ?>
