			...
			<form method="get" action="<?php echo esc_url($base_url); ?>" class="self-stretch flex flex-col sm:flex-row justify-between items-center gap-5" data-archive-filter-form data-search-min="3" data-search-debounce="350">
				<div class="flex justify-start items-center gap-5 flex-wrap">
					...
					<?php if (! empty($location_terms)) : ?>
						<div class="relative">
							...
							<select id="event-location-filter" name="location" class="appearance-none px-4 py-2 pr-8 text-neutral-800 text-base font-normal bg-transparent border-none cursor-pointer focus:outline-none" aria-label="Filter by location">
				...
				<div class="w-full sm:w-auto flex items-center gap-2.5">
					<div class="w-full sm:w-80 px-5 py-4 bg-white rounded-lg border border-zinc-300 flex justify-start items-center gap-2.5" role="search">
						...
						<input id="event-search" type="search" name="search" value="<?php echo esc_attr($current_search); ?>" placeholder="Search Events" class="flex-1 italic bg-transparent text-neutral-500 text-base font-light placeholder-neutral-500 focus:outline-none" />
					</div>
					<?php if ($has_filters) : ?>
