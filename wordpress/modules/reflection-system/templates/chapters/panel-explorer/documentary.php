				...
				<div class="reci-panel-scroll">
					<div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
						<?php foreach ((array) $args['items'] as $item) : ?>
							...
							$panel_annotations_json = wp_json_encode($panel_annotations, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
							?>
							<article class="overflow-hidden rounded-[20px] border border-[color:var(--reflection-border-soft)] bg-[var(--reflection-surface)]">
								<img class="panel-image block max-h-[480px] w-full cursor-zoom-in object-contain" src="<?php echo esc_url($item['src']); ?>" alt="<?php echo esc_attr($item['alt']); ?>" data-annotations="<?php echo esc_attr($panel_annotations_json); ?>" data-annotations-json="<?php echo esc_attr(base64_encode((string) $panel_annotations_json)); ?>">
								<div class="p-4">
