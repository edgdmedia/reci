    ...
    if (completeBtn && completeBtn instanceof HTMLElement) {
      ...
      const saveReflection = () => {
        if (window.reciDashboard && window.reciDashboard.restNonce) {
          ...
          .then((data) => {
            // The stage family saves and immediately shows its success panel,
            // so the share request has to be chained here rather than left to
            // a separate control the reader never returns to.
            const intent = window.reciReadShareIntent
              ? window.reciReadShareIntent(completeBtn)
              : { share: false, anonymous: false };

            if (intent.share && data && data.id && window.reciPatchJournalShare) {
              return window.reciPatchJournalShare(data.id, true, intent.anonymous)
                .catch((shareError) => {
                  // Never lose the save because the share failed.
                  console.error('Failed to share reflection:', shareError);
                })
                .then(() => {
