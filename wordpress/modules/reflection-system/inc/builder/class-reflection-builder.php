    ...
    class RECI_Reflection_Builder
    {
        ...
        public static function render_page(): void
        {
            ...
            $builder_config = [
                ...
                'styles' => function_exists('reci_reflection_system_styles') ? reci_reflection_system_styles() : [],
                ...
                'renderEndpoint' => rest_url('reci/v1/render-chapter'),
            ];
            ...
            <head>
                ...
                <?php wp_head(); ?>
                <style>
                    html, body { margin: 0 !important; padding: 0 !important; height: 100% !important; overflow: hidden !important; background: #f0f0f1; }
                    #reci-builder-shell { display: flex; flex-direction: column; height: 100vh; position: fixed; top: 0; left: 0; right: 0; bottom: 0; z-index: 99999; }
                    #reci-builder-topbar {
                    ...
                    #reci-builder-topbar .help-btn {
                        background: transparent; color: #ccc; border: 1px solid #555; border-radius: 3px;
                        padding: 6px 14px; font-size: 13px; cursor: pointer; margin-left: auto; margin-right: 8px;
                    }
                    #reci-builder-topbar .help-btn:hover { background: #333; color: #fff; }
                    #reci-builder-root { flex: 1; overflow: hidden; }

                    /* Help Modal Styles */
                    #reci-builder-help-modal {
                        display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%;
                        background: rgba(0,0,0,0.6); z-index: 100000; align-items: center; justify-content: center;
                    }
                    #reci-builder-help-modal.active { display: flex; }
                    .reci-help-modal-content {
                        background: #fff; border-radius: 8px; width: 600px; max-width: 90%; max-height: 80vh;
                        overflow-y: auto; box-shadow: 0 10px 25px rgba(0,0,0,0.2); font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Oxygen-Sans, Ubuntu, Cantarell, "Helvetica Neue", sans-serif;
                    }
                    .reci-help-modal-header {
                        padding: 16px 24px; border-bottom: 1px solid #e5e5e5; display: flex; justify-content: space-between; align-items: center;
                    }
                    .reci-help-modal-header h2 { margin: 0; font-size: 18px; color: #1e1e1e; }
                    .reci-help-modal-close {
                        background: none; border: none; font-size: 24px; line-height: 1; cursor: pointer; color: #666; padding: 0;
                    }
                    .reci-help-modal-close:hover { color: #d63638; }
                    .reci-help-modal-body { padding: 24px; color: #3c434a; font-size: 14px; line-height: 1.6; }
                    .reci-help-modal-body h3 { margin-top: 0; font-size: 15px; margin-bottom: 8px; }
                    .reci-help-modal-body p { margin-top: 0; margin-bottom: 16px; }
                    .reci-help-modal-body ul { margin-top: 0; margin-bottom: 16px; padding-left: 20px; }
                    .reci-help-modal-body li { margin-bottom: 4px; }
                </style>
            </head>
            <body>
            <div id="reci-builder-shell">
                <div id="reci-builder-topbar">
                    ...
                    <button type="button" class="help-btn" id="reci-builder-help-btn" title="How to use the Reflection Builder">
                        ? Help
                    </button>
                ...
                <!-- Help Modal -->
                <div id="reci-builder-help-modal">
                    <div class="reci-help-modal-content">
                        <div class="reci-help-modal-header">
                            <h2>How to Use the Reflection Builder</h2>
                            <button type="button" class="reci-help-modal-close" id="reci-builder-help-close">&times;</button>
                        </div>
                        <div class="reci-help-modal-body">
                            <h3>Overview</h3>
                            <p>The Reflection Builder allows you to create immersive, interactive stories composed of multiple <strong>Chapters</strong>. Each chapter represents a "scene" or section of your reflection.</p>
                            
                            <h3>Adding Chapters</h3>
                            <ul>
                                <li>Click <strong>+ Add Chapter</strong> to insert a new scene.</li>
                                <li>Choose a template (e.g., Progressive Text, Horizontal Panels, Hotspot Stage).</li>
                                <li>Drag and drop chapters in the left sidebar to reorder them.</li>
                            </ul>

                            <h3>Editing Content</h3>
                            <ul>
                                <li>Select a chapter from the sidebar to view its settings.</li>
                                <li>Use the <strong>Properties</strong> panel on the right to edit text, upload media, and configure interactions.</li>
                                <li>Changes are instantly previewed in the center stage.</li>
                            </ul>

                            <h3>Saving</h3>
                            <p>Click the blue <strong>Save</strong> button in the top right when you are finished. Your changes will be saved directly to the WordPress database.</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <script>
            // Help Modal Logic
            const helpBtn = document.getElementById('reci-builder-help-btn');
            const helpModal = document.getElementById('reci-builder-help-modal');
            const helpClose = document.getElementById('reci-builder-help-close');
        ...
        private static function enqueue_builder_assets(int $post_id): void
        {
            ...
            $js_path  = get_template_directory() . '/modules/reflection-system/assets/js/builder.js';
            ...
            wp_enqueue_script(
                'reci-reflection-builder',
                get_template_directory_uri() . '/modules/reflection-system/assets/js/builder.js',
                [],
                $version,
                true
            );
        ...
        public static function ajax_save_blueprint(): void
        {
            ...
            if (class_exists('RECI_Reflection_Content_Service')) {
                RECI_Reflection_Content_Service::invalidate_cache($post_id);
            }
