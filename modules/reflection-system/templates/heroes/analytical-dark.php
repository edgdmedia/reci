<?php
/**
 * Reflection hero variant: analytical, dark card.
 *
 * Pairs with the data-cards variant of the same name: the overlay washes the
 * background image toward the stage colour so the palette's own heading and
 * body colours stay readable on top of it.
 *
		...
		<div class="relative z-10 flex flex-col min-h-screen px-5 py-24 sm:px-6 lg:px-12 xl:px-20 <?php echo esc_attr($args['align_h_class']); ?> <?php echo esc_attr($args['align_v_class']); ?> <?php echo esc_attr($args['align_text_class']); ?>">
			<div class="w-full max-w-3xl">
				...
				<?php if (! empty($args['caption'])) : ?>
					<p class="mt-5 max-w-[58rem] text-sm leading-7 reci-reflection-muted"><?php echo reci_reflection_format_text($args['caption']); ?></p>
				<?php endif; ?>
				<?php if ($args['actions']) : ?>
