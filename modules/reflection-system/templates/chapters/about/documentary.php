	...
	<div class="reci-stage-shell">
		<div class="reci-stage-body">
			<div class="grid gap-4">
				...
				<p class="max-w-[74rem] text-base leading-8 reci-reflection-soft-text sm:text-[1.05rem] sm:leading-9"><?php echo reci_reflection_format_text($args['intro']); ?></p>
			</div>
					...
					<?php foreach ((array) $args['items'] as $item) : ?>
						<article class="rounded-3xl border border-[color:var(--reflection-border-soft)] bg-gradient-to-b from-[var(--reflection-card-strong)] to-[var(--reflection-card)] p-6">
							...
							<?php foreach ((array) ($item['paragraphs'] ?? []) as $paragraph) : ?>
								<p class="<?php echo $paragraph !== (($item['paragraphs'] ?? [])[0] ?? null) ? 'mt-4 ' : ''; ?>text-base leading-8 reci-reflection-soft-text"><?php echo reci_reflection_format_text($paragraph); ?></p>
							<?php endforeach; ?>
						</article>
					<?php endforeach; ?>
				</div>
