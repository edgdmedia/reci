...
$body_paragraphs = array_values(array_filter(array_map('trim', preg_split('/\n\s*\n/', (string) $args['body']) ?: [(string) $args['body']])));
?>
			...
			<div class="grid gap-6 lg:grid-cols-[minmax(0,2fr)_minmax(0,3fr)] lg:items-center">
				<?php if ($media_first) : ?>
					<div class="overflow-hidden rounded-[2rem] border border-[color:var(--reflection-border-soft)] bg-[var(--reflection-surface)] shadow-[0_24px_60px_rgba(0,0,0,0.18)]">
						<img class="block w-full max-h-[72vh] object-contain" src="<?php echo esc_url($args['image']); ?>" alt="<?php echo esc_attr($args['image_alt']); ?>">
						<?php if ($args['caption']) : ?><div class="px-5 py-4 text-xs leading-7 reci-reflection-muted"><?php echo reci_reflection_format_text($args['caption']); ?></div><?php endif; ?>
					</div>
				<?php endif; ?>
				<div class="max-h-[70vh] overflow-y-auto rounded-[2rem] border border-[color:var(--reflection-border-soft)] bg-gradient-to-b from-[var(--reflection-card-strong)] to-[var(--reflection-card)] p-6 sm:p-8 lg:p-10 [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-[color:var(--reflection-border)]">
					...
					<?php if ($body_paragraphs) : ?>
						<?php foreach ($body_paragraphs as $paragraph) : ?>
							<p class="mt-5 text-base leading-8 reci-reflection-soft-text sm:text-lg sm:leading-9"><?php echo reci_reflection_format_text($paragraph); ?></p>
						<?php endforeach; ?>
					<?php endif; ?>
					<?php if ($args['note']) : ?><div class="mt-6 rounded-[1.25rem] border border-[color:var(--reflection-border-soft)] bg-[var(--reflection-panel)] px-5 py-4 text-sm leading-7 reci-reflection-soft-text"><?php echo reci_reflection_format_text($args['note']); ?></div><?php endif; ?>
					<div class="mt-8 flex flex-wrap gap-4">
						...
						<?php endforeach; elseif (($args['transition_mode'] ?? 'button') === 'button' && !empty($args['continue_target']) && $args['continue_target'] !== '#') : ?>
				...
				<?php if (! $media_first) : ?>
					<div class="overflow-hidden rounded-[2rem] border border-[color:var(--reflection-border-soft)] bg-[var(--reflection-surface)] shadow-[0_24px_60px_rgba(0,0,0,0.18)]">
						<img class="block w-full max-h-[72vh] object-contain" src="<?php echo esc_url($args['image']); ?>" alt="<?php echo esc_attr($args['image_alt']); ?>">
						<?php if ($args['caption']) : ?><div class="px-5 py-4 text-sm leading-7 reci-reflection-muted"><?php echo reci_reflection_format_text($args['caption']); ?></div><?php endif; ?>
					</div>
				<?php endif; ?>
			</div>
		</div>
