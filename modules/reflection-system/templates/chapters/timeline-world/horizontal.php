			...
			<?php foreach ((array) $args['items'] as $index => $item) : ?>
				<div class="reci-timeline-panel" data-timeline-index="<?php echo esc_attr((string) $index); ?>">
					<article class="reci-timeline-card max-h-[76vh] overflow-y-auto">
						...
						<p class="mt-5 max-w-[56rem] text-lg leading-8 reci-reflection-soft-text"><?php echo reci_reflection_format_text($item['body'] ?? ''); ?></p>
						...
						<?php if (! empty($item['link'])) : ?>
							<p class="mt-5"><a class="font-semibold reci-reflection-accent no-underline" href="<?php echo esc_url($item['link']['href']); ?>" target="_blank" rel="noopener"><?php echo reci_reflection_format_text($item['link']['label']); ?></a></p>
						<?php endif; ?>
					</article>
				</div>
			<?php endforeach; ?>
