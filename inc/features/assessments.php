...
// CPT registration moved to custom tables
...
if (! function_exists('reci_media_hub_normalize_assessment_question')) {
	function reci_media_hub_normalize_assessment_question(array $raw_question, int $index = 0): array
	{
		...
		if (is_string($correct_answers)) {
			$correct_answers = reci_media_hub_parse_multiline_list($correct_answers);
		}
	...
	function reci_media_hub_calculate_assessment_score(array $questions, array $answers): array
	{
		...
		foreach ($questions as $question) {
			...
			if (is_array($correct) && ! empty($correct)) {
				...
				$correct_clean = array_values(array_filter(array_map('sanitize_text_field', $correct)));
				
				sort($user);
				sort($correct_clean);

				if (! empty($user) && $user === $correct_clean) {
					$score++;
				}
			}
		}
...
if (! function_exists('reci_media_hub_assessment_recommendations')) {
	function reci_media_hub_assessment_recommendations(array $topic_slugs = [], int $limit = 6): array
	{
		...
		$learn_posts = get_posts($query_args);
		...
		if ($remaining > 0) {
			...
			if (! empty($tax_query)) {
				$fallback_query_args['tax_query'] = $tax_query;
			}
...
if (! function_exists('reci_media_hub_get_assessment_submission_result')) {
	function reci_media_hub_get_assessment_submission_result(int $assessment_id, array $answers): array
	{
		...
		error_log(sprintf('RECI Debug: Scoring Result - ID: %d, Score: %d/%d (%d%%)', $assessment_id, $score['score'], $score['max_score'], $score['percentage']));

		// If we have a matching band, use its topics.
		if (! empty($ranges)) {
		...
		// Fallback: If no band matched, use the quiz's own topic tags for recommendations.
		$quiz_topics = wp_get_post_terms($assessment_id, 'reci_topic', ['fields' => 'slugs']);
		if (! is_wp_error($quiz_topics) && ! empty($quiz_topics)) {
			$result['recommendations'] = reci_media_hub_assessment_recommendations($quiz_topics);
		}

		// Fallback: If no recommendations yet, pull latest from Learn
		if (empty($result['recommendations'])) {
			$result['recommendations'] = reci_media_hub_assessment_recommendations([], 3);
			// Force them to load even with empty topics
		}
...
if (! function_exists('reci_media_hub_create_assessment_submission')) {
	function reci_media_hub_create_assessment_submission(int $assessment_id, array $answers): int
	{
		...
		$result = reci_media_hub_get_assessment_submission_result($assessment_id, $answers);

		$table = $wpdb->prefix . 'reci_assessment_submissions';
		...
		if (! $inserted) {
			error_log('RECI assessment submission insert failed.');
			return 0;
		}
...
if (! function_exists('reci_media_hub_get_assessment_submission_state')) {
	function reci_media_hub_get_assessment_submission_state(int $post_id): array
	{
		...
		if ($query_submission > 0 && (int) get_post_meta($query_submission, '_reci_submission_assessment_id', true) === (int) $post_id) {
