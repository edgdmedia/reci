 ...
 *
 * Usage via shortcode:
 *   [reci_post_listing post_type="post" posts_per_page="4" orderby="date" order="DESC" listing_style="content_compact"]
 */
	...
	class RECI_Media_Hub_Listing_Builder {
		...
		public static function shortcode($atts = []): string {
			$atts = shortcode_atts(
				[
					...
					'pagination_wrapper_class' => 'mt-8 flex w-full items-center justify-center gap-2',
		...
		protected static function normalize_config(array $config): array {
			$defaults = [
				...
				'pagination_wrapper_class' => 'mt-8 flex w-full items-center justify-center gap-2',
				'pagination_item_class'    => 'inline-flex items-center justify-center min-w-10 h-10 px-3 rounded-lg border border-zinc-300 text-sm font-medium text-neutral-800 hover:bg-zinc-100',
				'pagination_current_class' => 'inline-flex items-center justify-center min-w-10 h-10 px-3 rounded-lg bg-[#003594] text-sm font-medium text-white',
		...
		protected static function map_content_item(WP_Post $post, array $config): array {
			...
			return [
				...
				'sphere_terms'     => reci_get_post_spheres($post_id),
				'category'         => $category,
		...
		protected static function post_type_ui(string $post_type): array {
			$map = [
				...
				'reci_article'   => ['label' => 'Article',    'badge' => 'bg-reci-blue', 'text' => 'text-white'],
				'reci_podcast'   => ['label' => 'Podcast',    'badge' => 'bg-purple-600', 'text' => 'text-white'],
				'reci_video'     => ['label' => 'Video',      'badge' => 'bg-red-600', 'text' => 'text-white'],
				'reci_event'     => ['label' => 'Event',      'badge' => 'bg-amber-400', 'text' => 'text-neutral-800'],
				'reci_reflection'=> ['label' => 'Reflection', 'badge' => 'bg-teal-600', 'text' => 'text-white'],
		 ...
		 */
			...
			if ( in_array( $post_type, [ 'reci_course', 'reci_assessment', 'reci_author', 'reci_document' ], true ) ) {
				...
				$read_time = (string) get_post_meta($post_id, '_post_read_time_label', true);
				if (! $read_time) {
