...
if (! class_exists('RECI_Related_Posts_Service')) {
    class RECI_Related_Posts_Service {
        ...
        /**
         * Get cross-post-type related posts based on category, SDGs, and RECI Spheres.
         *
         * @param int                 $post_id Post ID.
         * @param array<string,mixed> $args    Args.
         * @return array<int,array<string,mixed>>
         */
        public static function get_cross_type_related(int $post_id, array $args = []): array {
            if ($post_id <= 0) {
                return [];
            }

            $post_types = $args['post_type'] ?? ['post', 'reci_video', 'reci_podcast', 'reci_event', 'reci_course'];
            ...
            $taxonomies_to_check = ['category', 'sdgs', 'reci_sphere'];
            ...
            if (count($tax_queries) > 1) {
                $base_query['tax_query'] = $tax_queries;
            }
